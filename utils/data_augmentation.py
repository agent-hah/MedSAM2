...
class RandomRotate90:
    ...
    def __call__(self, m):
        assert m.ndim == 3, 'Supports only 2D+time (SxHxW) images'
...
class RandomRotate:
    ...
    def __init__(self, random_state, angle_spectrum=30,  mode='reflect', order=0, **kwargs):
    ...
    def __call__(self, m):
        angle = self.random_state.randint(-self.angle_spectrum,
                                          self.angle_spectrum)
        if m.ndim == 3:
            m = rotate(m, angle, axes=(1, 2), reshape=False,
                       order=self.order, mode=self.mode, cval=-1)
        else:
            m = rotate(m, angle, axes=(0, 1), reshape=False,
                       order=self.order, mode=self.mode, cval=-1)
    ...
    def __call__(self, m):
        if self.random_state.uniform() < self.execution_probability:
            ...
            for c in range(m.shape[0]):
                ...
                result = self.mean + alpha * (m[c] - self.mean)
                # test
                seqs.append(result)
            m = np.array(seqs)
            m = np.clip(m, -1, 1)
...
class ElasticDeformation:
    ...
    def __init__(self, random_state, spline_order, alpha=2000, sigma=50, execution_probability=0.2,
                 **kwargs):
        """
    ...
    def __call__(self, m):
        if self.random_state.uniform() < self.execution_probability:
            ...
            dy, dx = [
                gaussian_filter(
                    self.random_state.randn(
                        1, volume_shape[1], volume_shape[2]),
                    # a,
            ...
            z, y, x = np.meshgrid(np.arange(z_dim), np.arange(
                y_dim), np.arange(x_dim), indexing='ij')
            indices = z + dz, y + dy, x + dx

            m = map_coordinates(
                m, indices, order=self.spline_order, mode='reflect')
            # if m.shape[0] == 1:
            #     m = m[0]
...
class AbstractLabelToBoundary:
    ...
    def __call__(self, m):
        ...
        boundary_arr = [np.where(np.abs(convolve(m, kernel)) > 0, 1, 0)
                        for kernel in kernels]
        channels = np.stack(boundary_arr)
        results = []
        if self.aggregate_affinities:
            assert len(
                kernels) % 3 == 0, "Number of kernels must be divided by 3 (one kernel per offset per Z,Y,X axes"
            ...
            for i in range(0, len(kernels), 3):
                ...
                xyz_aggregated_affinities = np.logical_or.reduce(
                    channels[i:i + 3, ...]).astype(np.int32)
            ...
            results = [_recover_ignore_index(
                channels[i], m, self.ignore_index) for i in range(channels.shape[0])]
...
class StandardLabelToBoundary:
    ...
    def __call__(self, m):
        ...
        if self.foreground:
            ...
            results.append(_recover_ignore_index(
                foreground, m, self.ignore_index))
...
class RandomLabelToAffinities(AbstractLabelToBoundary):
    ...
    def __init__(self, random_state, max_offset=10, ignore_index=None, append_label=False, z_offset_scale=2, **kwargs):
        super().__init__(ignore_index=ignore_index,
                         append_label=append_label, aggregate_affinities=False)
        self.random_state = random_state
...
class LabelToAffinities(AbstractLabelToBoundary):
    ...
    def __init__(self, offsets, ignore_index=None, append_label=False, aggregate_affinities=False, z_offsets=None,
        ...
        assert isinstance(offsets, list) or isinstance(
            offsets, tuple), 'offsets must be a list or a tuple'
        ...
        if z_offsets is not None:
            assert len(offsets) == len(
                z_offsets), 'z_offsets length must be the same as the length of offsets'
        else:
...
class Standardize:
    ...
    def __call__(self, m):
        ...
        return (m - mean) / np.clip(std, a_min=self.eps, a_max=None) # type: ignore
...
class AdditiveGaussianNoise:
    def __init__(self, random_state, scale=(0., 0.1), execution_probability=0.1, **kwargs):
        self.execution_probability = execution_probability
        self.random_state = random_state
        self.scale = scale
...
class AdditivePoissonNoise:
    def __init__(self, random_state, lam=(0.9, 1), execution_probability=0.1, **kwargs):
        self.execution_probability = execution_probability
        self.random_state = random_state
        self.lam = lam
...
class Relabel:
    ...
    def __call__(self, m):
        ...
        _, unique_labels = np.unique(m, return_inverse=True) # type: ignore
        result = unique_labels.reshape(m.shape) # type: ignore
