...
# Determine the best available hardware exactly once when the file loads
...
def to_cuda(data, non_blocking=True):
    """
    Moves data to the best available hardware safely.
    (Keeps the 'to_cuda' name so you don't have to rewrite the rest of your codebase!)
    """
    if isinstance(data, list):
        return [i.to(target_device, non_blocking=non_blocking) for i in data]
    elif isinstance(data, tuple):
        return tuple(i.to(target_device, non_blocking=non_blocking) for i in data)
    elif isinstance(data, dict):
        return {k: v.to(target_device, non_blocking=non_blocking) for k, v in data.items()}
    else:
        # Assumes it is a standard PyTorch Tensor
