"""
Preprocess HumanData DICOM files into MedSAM2-compatible NPZ format.
...
Usage:
    python preprocess_humandata.py \
        --input_dir data/medsam_preprocessed/HumanData/HumanData \
        --output_dir data/medsam_preprocessed/HumanData_NPZ \
        --image_size 512
"""
...
def derive_npz_name_from_path(rel_path: str, stem: str = "") -> str:
    """Build a descriptive NPZ filename from a path relative to the HumanData root.
    """
...
def preprocess_humandata(input_dir: str, output_dir: str, image_size: int, dsa_only: bool = True):
    """Walk the HumanData directory tree, convert all DICOMs to NPZ files."""
    ...
    # Pre-filter directories that have 'dsa' in path or filenames if required
    target_dirs = []
    ...
    if zip_files:
        ...
        if target_zips:
            ...
            desc = "ZIP files (DSA only)" if dsa_only else "ZIP files"
            for zip_path in tqdm(target_zips, desc=desc):
    ...
    # ── Summary ──────────────────────────────────────────────────────────
    print("\n" + "=" * 55)
    print("      HUMANDATA PREPROCESSING SUMMARY      ")
    print("=" * 55)
...
if __name__ == "__main__":
    ...
    preprocess_humandata(args.input_dir, args.output_dir, args.image_size, not args.process_all)
